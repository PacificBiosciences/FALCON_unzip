...
def run_track_reads(exe_pool, base_dir, file_list, min_len, bestn, db_fn):
    ...
    #rid_to_oid = open(os.path.join(rawread_dir, 'rawread_ids')).read().split('\n')
...
to the contigs with read_to_contig_map generated by `fc_get_read_hctg_map` in `4-quiver/read_maps/read_to_contig_map`',
